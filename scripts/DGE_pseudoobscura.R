...
## Additional method. LIMMA+VOOM
library(edgeR) # Loads LIMMA dependency
dps_DGE <- DGEList(dps_miR_pre_myRNA) # build
dps_DGE<-calcNormFactors(dps_DGE)	# TMM NORM
# Voom transformation
batch <- c("A","B","A","B")
sex <- c("female","female","male","male")
mm <- model.matrix(~0 + batch + sex) # Modelling step
v <- voom(dps_DGE, mm, plot = FALSE)
# LIMMA DGE, Empirical Bayes
fit <- lmFit(v, mm)
fit <- eBayes(fit)
dge_voom <- topTable(fit, coef=ncol(mm), n = Inf)

# Compare both DGE sets
dge_miR_Dq2 <- rownames(resPreB[which((resPreB$padj<0.10) & (abs(resPreB$log2FoldChange) >= log(1.25))),])
dge_Vm  <- dge_voom[which((dge_voom$adj.P.Val<0.10) & (abs(dge_voom$logFC) >= log(1.25))),]
Voom_specific <- row.names(resPreB[names(which(table(c(intersect(dge_miR_Dq2,rownames(dge_Vm)),rownames(dge_Vm)))==1)),])
#  [1] "dps_15"        "dps_18"        "dps_3420"      "dps-mir-137"  
#  [5] "dps-mir-2507b" "dps-mir-2509"  "dps-mir-2523"  "dps-mir-2527" 
#  [9] "dps-mir-2b-1"  "dps-mir-304"   "dps-mir-92c"   "dps-mir-932"  
# [13] "dps-mir-959"   "dps-mir-989" 
Dq2_specific <- row.names(dge_voom[names(which(table(c(intersect(dge_miR_Dq2,rownames(dge_Vm)),dge_miR_Dq2))==1)),])
# [1] "dps-bantam"   "dps-mir-263a" "dps-mir-276a" "dps-mir-277"  "dps-mir-284" 
# [6] "dps-mir-317"  "dps-mir-958" 
in_both <- dge_miR_Dq2[!dge_miR_Dq2 %in% Dq2_specific]
#  [1] "dps-mir-2510" "dps-mir-2520" "dps-mir-2536" "dps-mir-2552" "dps-mir-2563"
#  [6] "dps-mir-276c" "dps-mir-306"  "dps-mir-311"  "dps-mir-318"  "dps-mir-92a" 
# [11] "dps-mir-92b"  "dps-mir-964"  "dps-mir-994"  "dps-mir-9b"   "dps-mir-9c"



# Compare log2FC
DGE_joint <- merge(as.data.frame(resPreB),dge_voom,by = 0)
pdf("plots/dpse_DGE_l2FC_two_methods.pdf")
plot(DGE_joint$log2FoldChange, DGE_joint$logFC, pch = 19, xlab = "log2FC DESeq2", ylab = "log2FC limma+Voom", col = 'grey')
abline(a=0, b=1, lty = 'dashed', col = "blue")
abline(lm(DGE_joint$logFC ~ DGE_joint$log2FoldChange), col = 'darkgrey')
row.names(DGE_joint) <- DGE_joint$Row.names
points(DGE_joint[in_both,]$log2FoldChange, DGE_joint[in_both,]$logFC, pch = 19, col = 'black')
points(DGE_joint[Dq2_specific,]$log2FoldChange, DGE_joint[Dq2_specific,]$logFC, pch = 19, col = 'darkred')
points(DGE_joint[Voom_specific,]$log2FoldChange, DGE_joint[Voom_specific,]$logFC, pch = 19, col = 'orange')
dev.off()

# Save results as R objects
save(dge_voom, file = "results/dpse_DGE_voom.Rdata")
