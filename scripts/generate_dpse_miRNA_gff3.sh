...
curl -O "ftp://mirbase.org/pub/mirbase/CURRENT/genomes/dps.gff3"
...
for file in 12flies/12FliesAlignments/dps*.html; do cat $file | grep -A 6 -e '/dps' | perl -ne '$_ =~ s/\<\/SPAN\>//g;$_ =~ s/<SPAN class=\"[a-zA-Z]+\"\>//g; print $_;' | grep -e'dps' -e'<td nowrap>' | perl -ne '$_ =~ s/\<.*\"\>//g; $_ =~ s/\<\/.*\>//g; $_ =~ s/\<td nowrap\>\s+//g;print $_;' | awk '{print $1}'; done | perl -ne 'if($_ =~ /^dps/){print ">".$_}else{$_ =~ s/\-//g; print $_;}' | tac | perl -e '$aa=join("",<>); $aa =~ s/\n\>/\t/g; print $aa;' | awk '{print $2 "\t" $1}' | grep -e '^dps_'| sort | uniq | perl -e '%out = ( ); @lines = split("\n",join("\n",<>)); for(@lines){@row = split("\t",$_); $out{$row[0]}=$row[1];}; for(keys %out){print ">".$_."\n".$out{$_}."\n";}' | grep -e'd' -e'T' > dps_newLai_pre.fa
