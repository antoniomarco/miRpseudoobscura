...
# NOTE RAW to avoid multiple correction errros
...
# Only one cluster contained Dpse microRNAs unmatched to putative Dmel orthologs: dps-mir-92c
...
# At least 60 nucleotides in the alignment
in.bed <- output.genome.blast[output.genome.blast$Alignment.Length>=60,c(2,9,10,1)]
...
# Only hits in assembled chromosomes
two.hits.out <- two.hits.out[two.hits.out$'Dmel CHR' %in% c("chrX", "chr2L", "chr2R", "chr3L", "chr3R", "chr4"),]
